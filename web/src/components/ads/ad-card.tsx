...
export function AdCard({ ad, boosted }: { ad: Ad; boosted?: boolean }) {
  ...
  return (
    /* The link itself never moves: it is what tracks `:hover`, so lifting it
     * would pull it out from under the cursor near its bottom edge and the
     * hover would flicker on and off. The sheet inside is what rises. */
    <Link href={`/ads/${ad.id}`} className="group relative block h-full">
      <div
        className={cn(
          "relative flex h-full flex-col overflow-hidden rounded-xl bg-card shadow-sm ring-1 transition-[transform,box-shadow] duration-300 ease-soft group-hover:-translate-y-1 group-hover:shadow-lg group-active:translate-y-0 group-active:shadow-md",
        ...
        <div className="flex flex-1 flex-col gap-3 p-4 sm:p-5">
          ...
          <div className="mt-auto flex items-end justify-between gap-3 border-t border-border pt-3">
            <p className="numeric font-heading text-xl font-bold tracking-tight text-primary">
              <span className="mr-0.5 text-sm font-semibold text-brand-600">
                ৳
              </span>
